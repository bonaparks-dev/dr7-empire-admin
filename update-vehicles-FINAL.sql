...
UPDATE vehicles SET daily_rate = 249, status = 'available', display_name = 'Mercedes V Class VIP DR7', metadata = '{"pricing": {"daily": 249, "ncc": "Su preventivo personalizzato"}, "notes": "Corse con conducente (Taxi/NCC) disponibili", "image": "/v-class.jpeg"}'::jsonb WHERE display_name LIKE 'Mercedes%Classe V%' OR display_name LIKE 'Mercedes-Benz Classe V%';
...
SELECT display_name, daily_rate, category, status, metadata->'pricing' as pricing FROM vehicles WHERE status = 'available' ORDER BY category, daily_rate;
